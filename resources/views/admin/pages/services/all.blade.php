  ...
  <div class="card">
    ...
    <div class="card-header card-header-stretch">
      ...
      <div class="card-title">
        <h3 class="m-0 text-gray-800">Services</h3>
      </div>
      <!--end::Title-->
      ...
      <div class="m-0 card-toolbar">
        ...
        <ul class="border-transparent nav nav-stretch fs-5 fw-semibold nav-line-tabs" role="tablist">
          <li role="presentation">
            <a href="{{ route('services.create') }}" class="btn btn-primary btn-sm">Add new</a>
          </li>
        </ul>
        <!--end::Tab nav-->
      </div>
          ...
          <table class="table align-middle table-row-bordered table-row-solid gy-4 gs-9">
            ...
            <tbody class="text-gray-600 fs-6 fw-semibold">
              @foreach ($services as $service)
                <tr>
                  <td class="ps-9">{{ $service->id }}</td>
                  <td class="ps-2"><img width="60px" height="60px" src="{{ asset($service->image) }}" alt="">
                  </td>
                  <td class="ps-2">{{ $service->name }}</td>
                  <td class="text-success">
                    <label class="switch">
                      <input type="checkbox" id="is_published" class="form-control" name="is_published"
                        value="{{ $service->is_published }}" {{ $service->is_published == 'on' ? 'checked' : '' }}>
                      <span class="slider round"></span>
                    </label>
                  </td>
                  <td class="gap-2 d-flex">
                    <a href="{{ route('services.edit', $service->id) }}" class="btn btn-sm btn-secondary">Edit</a>
